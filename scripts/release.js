...
function classify(subject){ const m = subject.match(/^(feat|fix|perf|refactor|docs|test|chore|style|ci|build)(\([^)]*\))?(!)?:/); if(m){ return {type:m[1],breaking:!!m[3]}; } return {type:'other',breaking:false}; }
function collectCommits(from, to){ const range = from ? `${from}..${to}` : to; const out = sh(`git log ${range} --pretty="format:%H|%s"`); return out.split(/\n/).filter(Boolean).map(l=>{const [h,s]=l.split('|'); return {hash:h,subject:s};}); }
function suggestNextVersion(prevTag, commits){ const prev = parseSemver(prevTag||'v0.0.0'); let level='patch'; let breaking=false; for(const c of commits){ const info=classify(c.subject); if(info.breaking) { breaking=true; break; } if(info.type==='feat') level = level==='patch' ? 'minor' : level; }
...
function group(commits){ const buckets={feat:[],fix:[],perf:[],refactor:[],docs:[],test:[],chore:[],style:[],other:[]}; for(const c of commits){ const info=classify(c.subject); buckets[info.type?info.type:'other'].push(c); } return buckets; }
function renderNotes(version, commits){ const buckets=group(commits); const order=['feat','fix','perf','refactor','docs','test','style','chore','other']; let out=`# ${version}\n\n`; for(const k of order){ if(!buckets[k].length) continue; const titles={feat:'新增',fix:'修复',perf:'性能',refactor:'重构',docs:'文档',test:'测试',style:'风格',chore:'杂项',other:'其他'}; out+=`## ${titles[k]}\n`; for(const c of buckets[k]) out+=`- ${c.subject}\n`; out+='\n'; } return out; }
